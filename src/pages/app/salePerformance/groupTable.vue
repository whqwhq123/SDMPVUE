      ...
      <view class="table">
        ...
        <block v-for="(item, index) in tableData.body" :key="index">
          <view class="tr" v-if="index % 2 == 0">
            ...
            >{{index+1}}</view>
            <view class="td overflow">{{item.performanceName}}</view>
            <view class="td">{{item.successCount}}</view>
...
export default {
  ...
  methods: {
    handleMore() {
      let { start: startTime, end: endTime, type } = this.tableData.time;
      wx.navigateTo({
        url: jumpUrl[this.tableData.type] + `?startTime=${startTime}&endTime=${endTime}&type=${type}`
      })

    },
    lookDetail(row) {
      wx.navigateTo({
        url: `/pages/app/salePerformanceDetail/main?foursId=${row.foursId}`
      })
    }
  }
}
