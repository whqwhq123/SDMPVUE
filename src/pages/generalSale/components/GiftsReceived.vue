  ...
  export default {
    ...
    computed: {
        showList:function(){
            if(this.showAll == false){   //不显示全部数据               
                var showList = [];　　　　　//空数组　　　　　　　　　　
                if(this.giftList.length > 3){　　　//只显示5条　　　　
                    for(var i=0;i<3;i++){
                        showList.push(this.giftList[i]); //将数组的前5条存放到showList数组中
                    }
                }else{
                    showList = this.giftList  //个数足够显示，不需要在截取
                }
                return showList;　　　　　　　　　　　　　　　　
            }else{  //显示全部数据
    ...
    methods: {
        zhankai() {
           ...
           this.$emit('fromChild1', this.showAll)
        },
        quickly() {
            wx.navigateTo({
                url: '../code/main',
